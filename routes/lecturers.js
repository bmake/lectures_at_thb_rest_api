...
module.exports = app => {
  ...
  app.get('/v1/lecturers',
    (request, response) => {
      const query = fs.readFileSync('queries/allLecturers.rq', 'utf8')
      ...
      axios.get(fuseki_url, {
        ...
        .then(res => {
          ...
          if (results.length === 0) {
            sendResponse(response, 404, 'No lecturer was found!', null)
          } else {
            const lecturer = results.map(r => extractValues(r))
            sendResponse(response, 200, null, lecturer)
          }
        })
